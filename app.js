...
const icon = {
  ...
  upload: `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21V9m0 0-4 4m4-4 4 4M4 5h16"/></svg>`,
};
...
function backupAllData() {
  const payload = { version: 1, exportedAt: new Date().toISOString(), tep: state.entries.tep, jeng: state.entries.jeng };
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url; a.download = `bp-log-backup-${new Date().toISOString().slice(0, 10)}.json`;
  document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
}
...
function handleRestoreFile(file) {
  ...
  reader.onload = () => {
    try {
      ...
      if (!data || (!Array.isArray(data.tep) && !Array.isArray(data.jeng))) {
        throw new Error("This doesn't look like a BP Log backup file.");
      }
      const tepList = Array.isArray(data.tep) ? data.tep : [];
      const jengList = Array.isArray(data.jeng) ? data.jeng : [];
      const proceed = confirm(
        `This backup has ${tepList.length} Tep reading(s) and ${jengList.length} Jeng reading(s), from ${data.exportedAt ? new Date(data.exportedAt).toLocaleDateString() : "an unknown date"}.\n\nThis will REPLACE what's currently on this device. Continue?`
      );
      if (!proceed) return;
      state.entries.tep = tepList;
      state.entries.jeng = jengList;
      persistEntries("tep", tepList);
      persistEntries("jeng", jengList);
      render();
      alert("Backup restored.");
    } catch (err) {
      alert("Couldn't restore this file: " + err.message);
    }
  };
...
function render() {
  ...
  <div class="header">
    <h1>BP Log</h1>
    <div class="row gap4">
      <button class="btn-ghost" data-action="backup">${icon.save} Backup</button>
  ...
  <div class="segmented mb8">
    ${["log", "weekly"].map((v) => `<button class="seg ${state.view === v ? "active" : ""}" data-action="view" data-id="${v}">${v === "log" ? "Log" : "Weekly"}</button>`).join("")}
  </div>
  <div class="row spread mb20">
    <button class="link" data-action="restore-trigger">${icon.upload} Restore from backup</button>
...
document.addEventListener("click", (e) => {
  ...
  else if (action === "backup") backupAllData();
  else if (action === "restore-trigger") triggerRestore();
